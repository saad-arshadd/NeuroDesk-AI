...
def command():
    ...
    with sr.Microphone() as source:
        ...
        print ("Listening ...",end="",flush=True) #automatically removes the "listening " wgen moving to next command
    ...
    try:
       print("\r",end="",flush=True) # to gap the command in the  next line
...
def cal_day():
    day=datetime.datetime.today().weekday()+1 # returns from 0(moday ) to 6 (sunday) so +1 because normally we refer as 1-7
    day_dict={ # add dict by your self because weekday only gives you number 
        1:"Monday",
        2:"Tuesday",
        3:"Wednesday",
        4:"Thursday",
    ...
    if day in day_dict.keys():
        day_of_week =day_dict[day] # number e agaist key se day ko fetch karlega dictionary se
        print(day_of_week)
    return day_of_week

def wishme() :#based on time
    hour = int(datetime.datetime.now().hour)
    t=time.strftime("%I:%M:%p")
    day = cal_day()
    
    if(hour >= 0 ) and ( hour <=12) and ('AM' in t):
        speak(f"Hello Saad and a very Good Morning , its {day} and the time is {t}")
    elif (hour>=12) and(hour <=16) and ('PM' in t):
        speak(f"Hello Saad and Good Afternoon , its {day} and the time is {t}")
    else:
        speak(f"Hello Saad and  Good Evening , its {day} and the time is {t}")
        
 
def social_media (command):
    if 'facebook' in command:
        speak("Roger that ,open your facebook")
        webbrowser.open("https://www.facebook.com/")
    elif 'whatsapp' in command:
        speak("Roger that ,open your Whatsapp")
        webbrowser.open("https://web.whatsapp.com/")
    elif 'discord' in command:
        speak("Roger that ,open your Discord Server")
        webbrowser.open("https://www.discord.com/")
    elif 'instagram' in command:
        speak("Roger that ,open your Instagram")
        webbrowser.open("https://www.instagram.com/")
    elif 'chat gpt' in command:
        speak("Roger that ,open Chat Gpt")
        webbrowser.open("https://chatgpt.com/")
    elif 'linkedin' in command:
        speak("Roger that ,open Linkedin")
        webbrowser.open("https://linkedin.com/")
    elif 'github' in command:
        speak("Roger that ,open Github")
        webbrowser.open("https://github.com/")
    elif 'wikipedia' in command:
        speak("Roger that ,open Wikipedia")
...
def schedule():
    day = cal_day().lower()
    speak("Okay Saad today's schedule is ")
    week={
    "monday": "Saad, from 9:00 to 9:50 you have Algorithms class, from 10:00 to 11:50 you have System Design class, from 12:00 to 2:00 you have a break, and today you have Programming Lab from 2:00 onwards.",
    "tuesday": "Saad, from 9:00 to 9:50 you have Web Development class, from 10:00 to 10:50 you have a break, from 11:00 to 12:50 you have Database Systems class, from 1:00 to 2:00 you have a break, and today you have Open Source Projects lab from 2:00 onwards.",
    "wednesday": "Saad, today you have a full day of classes. From 9:00 to 10:50 you have Machine Learning class, from 11:00 to 11:50 you have Operating Systems class, from 12:00 to 12:50 you have Ethics in Technology class, from 1:00 to 2:00 you have a break, and today you have Software Engineering workshop from 2:00 onwards.",
    "thursday": "Saad, today you have a full day of classes. From 9:00 to 10:50 you have Computer Networks class, from 11:00 to 12:50 you have Cloud Computing class, from 1:00 to 2:00 you have a break, and today you have Cybersecurity lab from 2:00 onwards.",
    "friday": "Saad, today you have a full day of classes. From 9:00 to 9:50 you have Artificial Intelligence class, from 10:00 to 10:50 you have Advanced Programming class, from 11:00 to 12:50 you have UI/UX Design class, from 1:00 to 2:00 you have a break, and today you have Capstone Project work from 2:00 onwards.",
    "saturday": "Saad, today you have a more relaxed day. From 9:00 to 11:50 you have team meetings for your Capstone Project, from 12:00 to 12:50 you have Innovation and Entrepreneurship class, from 1:00 to 2:00 you have a break, and today you have extra time to work on personal development and coding practice from 2:00 onwards.",
    "sunday": "Saad, today is a holiday, but keep an eye on upcoming deadlines and use this time to catch up on any reading or project work."
    }
    if day in week.keys():
        speak(week[day]) #will speak the schedule of the current day 
    

import os

def OpenApp(command):
    command = command.lower()  # optional: make it case-insensitive
    ...
    elif "visual code" in command or "vs code" in command:
...
def CloseApp(command):
    ...
    if "calculator" in command:
        ...
        os.system('powershell "Get-Process | Where-Object {$_.Name -like \'CalculatorApp\'} | Stop-Process -Force"')
        ...
        speak("Closingg Visual Studio Code")
...
def browsing(query):
    if 'google' in query:
        speak("Saad, What should I search in google?")
        s=command()
        webbrowser.open(f"{s}")
...
if __name__=="__main__":
    ...
    while True :
        ...
        query =command().lower()
        #query = input("Enter Your Command Please ..")
        #print(query)
        
        
        if ('facebook' in query ) or ('discord' in query ) or ('instagram' in query ) or ('whatsapp' in query ) or ('chat gpt' in query ) or ('linkedin' in query ) or ('github' in query ) or ('wikipedia' in query ) :
            social_media(query)
        ...
        elif("open calculator" in query) or ("open notepad" in query) or ("open visual code" in query) or ("open paint" in query) or ("open zoom" in query):
            OpenApp(query)
            
        elif("close calculator" in query) or ("close notepad" in query) or ("close visual code" in query) or ("close paint" in query) or ("close zoom" in query):
            CloseApp(query)
